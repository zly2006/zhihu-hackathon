...
将横向多姿态素材接入旧页面时，不能把雪碧图的裁切规则全局套到原有单人立绘上，也不能让体验模式继续引用旧资源。应先区分单人图与雪碧图，统一资源映射后再按单个姿态的画布比例裁切，并用实际页面截图检查标题页、剧情页和存档预览。

## 资产重构失败经验

原始横向三姿态角色表同时承载 normal、happy 和调皮三类基础表情，不能按“旧雪碧图”清理。任何拆分或重生成都必须先保留原图，确认每个姿态的语义和原始头身比例，再制作独立图与 atlas；若新图的比例或边缘与原图不一致，应回退并以原图为基准修正，不能用新生成图覆盖基准资产。

## 资产姿态集合误删经验

角色资源的姿态集合必须以历史资产、平台引用和配置共同核对，不能根据单次需求把集合缩减为三个名称。新增姿态与基础姿态都必须保留；清理前先建立完整清单并逐项核对引用，Atlas 也必须覆盖这份完整清单。
