...
> 日期：2026-08-20
>
> 当前状态：`IN_PROGRESS`，已完成契约与原文幂等入库纵向切片
> 第一版定义：可在授权试点数据上持续运行、可追溯、可更新、可删除、可审核的完整闭环；不是只有演示数据的聊天原型。

## 1. 一句话目标

把合法取得的知乎回答变成有原文证据的“决策卡片”，把相似卡片放进同一个“情景抽屉”，再从同一个抽屉里识别有不同选择和后续路径的“分叉路口”。

本阶段到“路口”结束，不回答用户应该选择哪条路。
...
| 卡片 | `DecisionEpisode` | 表达一次“处境—决策点—行动—后续观察” |
| 抽屉 | `CanonicalScenario` | 合并前置状态和决策点相近的卡片 |
| 路口 | `BranchPoint` | 同一抽屉下出现至少两种有证据的行动路径 |

这四层必须逐层成立：没有原文证据就没有正式卡片；没有可比较卡片就没有正式抽屉；没有不同选择及后续就没有正式路口。
...
- 一篇回答可以产生 0..N 张决策卡片，每个正式主张都能定位到同一快照中的证据片段。
- PostgreSQL 保存原文、证据、版本、审核和业务状态，是唯一权威数据源。
- Graphiti + Neo4j 保存可重建的时序语义图，图中的正式事实能映射回本地经历与证据。
...
| 决策卡片 | `decision_episode.v1` | 主张结构变化必须升版本并重跑黄金集 |
...
目标：把一篇回答稳定拆成 0..N 张有证据的决策卡片。

任务：

- [ ] `M2-01` 建立 `analysis_run/decision_episode/semantic_claim/claim_evidence` 及类型子表。
- [ ] `M2-02` 实现亲历、建议、反事实、转述和泛观点的资格分类。
- [ ] `M2-03` 按主体、时间和决策点切分经历边界；主体切换必须断开路径。
- [ ] `M2-04` 实现结构化抽取器接口，输出字段级置信度和 `evidence_span_ids`。
- [ ] `M2-05` 实现硬校验器：schema、枚举、偏移、同快照、证据覆盖、未知值和时间一致性。
- [ ] `M2-06` 失败分为可重试、模型不确定、需要人工判断和永久无经历四类。
- [ ] `M2-07` 建立低置信卡片审核任务和接受/修改/拒绝记录。
- [ ] `M2-08` 在黄金集上评估资格分类、边界、字段、证据和未知值误补。

退出门：

- `CONFIRMED` 主张 100% 具有同快照有效证据偏移；
- 黄金集证据支持准确率建议不低于 95%；
- 亲历识别 precision 建议不低于 90%，避免把建议当经历；
- 未知字段误补率不高于 1%；
- 多阶段、主体切换、反事实和没有经历的样本均有回归测试。

指标未达标时允许继续积累候选卡片，但不得进入正式情景和分叉。

### M3：Graphiti 时序图投影

目标：把已确认卡片安全投影为可查询、可删除、可重建的时序图。
...
- 已投影卡片 100% 保存 `decision_episode_id ↔ graphiti_episode_uuid`；
- 相同投影任务重放不产生重复正式关系；
...
目标：把可比较卡片放进同一抽屉，并只用明确证据串起单一叙事路径。
...
- 结果分化误报为分叉的数量为 0；
- 分叉支持数与去重后的来源数一致；
- 任意分支能在一次查询中返回卡片、快照和证据；
- 输出中没有“成功概率”“最佳选择”或未经证据支持的因果语言。

### M6：检索、审核和 API

目标：让内部使用者能找回答、找卡片、看抽屉、比较路径并审核路口。
...
- [ ] `M6-04` 实现内部审核界面或最小审核控制台；不建设终端用户推荐 UI。
- [ ] `M6-05` 实现角色权限：原始层、脱敏证据层、语义层和审核层分离。
- [ ] `M6-06` 记录谁在何时查看敏感证据、修改卡片、确认情景或审核分叉。
...
| `G-07` 输入有界 | 长回答先拆卡片；超限 episode 拒绝或进入拆分队列 |
...
| 检索回答、卡片和路径 | M6 | Recall@20、过滤正确率、证据回溯 |
...
| 模型成本或摄入延迟不可控 | 单卡片调用数、队列时间或失败率持续上升 | 限制 episode 大小；缓存不变输入；批量调度但不使用更新 bulk | 平台/安全 |
