...
export async function GET(request: Request) {
  ...
  if (
    ...
    !(avatarUrl.hostname === "zhimg.com" || avatarUrl.hostname.endsWith(".zhimg.com"))
  ) {
  ...
  try {
    ...
    if (!response.ok)
      return NextResponse.json(
        { error: `avatar upstream HTTP ${response.status}` },
        { status: 502 },
      );
    const contentType = response.headers.get("content-type") || "image/jpeg";
    if (!contentType.startsWith("image/"))
      return NextResponse.json({ error: "avatar upstream is not an image" }, { status: 502 });
