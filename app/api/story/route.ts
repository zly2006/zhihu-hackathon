...
import {z} from 'zod';
import {castPool,initial,choose,limits,publicState,requiredCastCount,total,type GameEvent,type CharacterProfile} from '../../../lib/story';
...
const schema=z.object({storyId:z.string().uuid().optional(),action:z.enum(['start','choose','retry','restart']),choice:z.number().int().min(0).max(limits.totalChoiceMax-1).optional(),expected:z.number().int().optional(),profiles:z.array(z.object({id:z.string(),name:z.string(),gender:z.enum(['男','女']),background:z.string().max(300).optional(),zhihuHandle:z.string().max(80).optional()}).strict()).length(requiredCastCount).optional()}).strict();
export async function POST(req:NextRequest) {
 ...
 if(action==='restart'||!state){
  ...
  if(!profiles)return NextResponse.json({error:`请先选择${requiredCastCount}位角色。`},{status:400});
  id=randomUUID();
  try{state=initial(profiles as CharacterProfile[]);}catch(error){return NextResponse.json({error:error instanceof Error?error.message:'角色选择无效。'},{status:400});}
 }
