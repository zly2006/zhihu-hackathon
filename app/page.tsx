...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    void fetch('/api/story').then(r=>r.json()).then(d=>{const catalog={pool:d.pool||[],backgrounds:d.backgrounds||[],requiredCastCount:d.requiredCastCount||4,maxStages:d.maxStages||7,defaultBackgroundId:d.defaultBackgroundId||'university'};const available=new Set(catalog.pool.map((member:PoolMember)=>member.id));const defaults=['m1','m3','f2','f4'].filter((id)=>available.has(id)).slice(0,catalog.requiredCastCount);setLiveCatalog(catalog);setSelectedBackgroundId(catalog.defaultBackgroundId);setSelectedIds((current)=>{const valid=current.filter((id)=>available.has(id));return valid.length===catalog.requiredCastCount?valid:defaults;});setCatalogReady(catalog.pool.length>=catalog.requiredCastCount);}).catch(()=>setCatalogReady(false));
