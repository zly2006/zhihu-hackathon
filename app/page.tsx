...
const defaults:Preferences={speed:35,delay:3,opacity:86,fontSize:25,motion:true};
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    void fetch('/api/story').then(r=>r.json()).then(d=>{const catalog={pool:d.pool||[],backgrounds:d.backgrounds||[],requiredCastCount:d.requiredCastCount||4,maxStages:d.maxStages||7,defaultBackgroundId:d.defaultBackgroundId||'university'};setLiveCatalog(catalog);setSelectedBackgroundId(catalog.defaultBackgroundId);setSelectedIds((current)=>current.length?current:['m1','m3','f2','f4'].filter((id)=>catalog.pool.some((member:PoolMember)=>member.id===id)).slice(0,catalog.requiredCastCount));}).catch(()=>{});
  ...
  useEffect(()=>{if(ready&&started&&state&&!waiting&&!busy)persist('lamplight_resume',{version:1,mode,storyId:storyId.current,state,line,chats,time:new Date().toISOString(),partner:partner.id} satisfies SaveSlot);},[state,line,chats,mode,ready,started,waiting,busy,partner.id]);
  ...
  async function act(action:'start'|'restart'|'retry'|'choose',choice?:number){
    ...
    try{
      const selectedProfiles=selectedIds.map((id)=>{const member=liveCatalog.pool.find((candidate)=>candidate.id===id);if(!member)throw new Error('角色资料已变化，请重新选择。');return {id:member.id,name:member.name,gender:member.gender,...(profiles[id]||{})};});
  ...
  function openSetup(){if(!auth?.authorized){notify('请先登录知乎。');return;}setSetupStep(0);setPanel('setup');}
  function toggleMember(id:string){setSelectedIds((current)=>current.includes(id)?current.filter((item)=>item!==id):current.length<liveCatalog.requiredCastCount?[...current,id]:current);}
  ...
  function canAdvanceSetup(){if(setupStep===0)return Boolean(selectedBackgroundId);if(setupStep===1)return selectedIds.length===liveCatalog.requiredCastCount;return selectedIds.length===liveCatalog.requiredCastCount&&playerName.trim().length>0;}
  function nextSetup(){if(!auth?.authorized){notify('请先登录知乎。');return;}if(!canAdvanceSetup())return;track('ui_setup_step',{step:setupStep});if(setupStep<2){setSetupStep((step)=>step+1);return;}setPanel(null);setMode('live');setState(null);setPartial(null);setLine(0);setChats({});setStarted(true);storyId.current=undefined;void act('start');}
  ...
  async function saveSlot(index:number){
    ...
    try{let id=storyId.current;if(mode==='live'){const r=await fetch('/api/saves',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storyId:id})}),d=await r.json();if(!r.ok)throw new Error(d.error);id=d.storyId;}
      const next=[...slots];next[index]={version:1,mode,storyId:id,state:structuredClone(state),line,chats:structuredClone(chats),time:new Date().toISOString(),partner:partner.id};if(persist('lamplight_slots',next)){setSlots(next);setOverwrite(null);notify(`已保存到位置 ${String(index+1).padStart(2,'0')}`);}
  ...
  async function loadSlot(slot:SaveSlot){
    ...
    try{let id=slot.storyId,loaded=slot.state;if(slot.mode==='live'){const r=await fetch('/api/saves',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storyId:id})}),d=await r.json();if(!r.ok)throw new Error(d.error);id=d.storyId;loaded=d.state;}
      storyId.current=id;setState(structuredClone(loaded));setPartial(null);setMode(slot.mode);setLine(Math.min(slot.line,Math.max(0,(loaded.nodes.at(-1)?.lines.length||1)-1)));setShown(0);setChats(structuredClone(slot.chats));setError('');setPlayMode('manual');setStarted(true);setPanel(null);notify('已回到保存的那一刻');
  ...
  return <main className={`game-shell ${preferences.motion?'':'reduce-motion'}`}>
    ...
    <section className="game-stage" style={css} aria-label="留一盏灯，视觉小说舞台" onContextMenu={e=>{if(!(e.target instanceof HTMLElement&&e.target.closest('input,textarea'))){e.preventDefault();if(!chatOpen)setPanel(panel?null:'menu');}}}>
      ...
      <dialog ref={dialogRef} className={`game-dialog panel-${panel}`} onCancel={e=>{e.preventDefault();setPanel(null);}} onClick={e=>{if(e.target===e.currentTarget)setPanel(null);}} aria-label={panel==='setup'?'开始正式故事':panel==='history'?'回看故事':panel==='settings'?'游戏设置':panel==='save'?'保存故事':panel==='load'?'读取故事':'游戏菜单'}><div className="modal-card"><Control icon="close" className="modal-close" aria-label="关闭弹窗" onClick={()=>setPanel(null)}/><span className="eyebrow">LAMPLIGHT · {panel==='setup'?'NEW STORY':panel==='history'?'BACKLOG':panel==='settings'?'PREFERENCES':panel==='save'?'SAVE':panel==='load'?'LOAD':'PAUSE'}</span>
        ...
        {panel==='settings'&&<><h2>按照你的节奏</h2><div className="settings-list"><label>文字显示速度 <output>{preferences.speed===0?'立即显示':`${preferences.speed} 字 / 秒`}</output><input type="range" min="0" max="80" step="5" value={preferences.speed} onChange={e=>setPreferences(p=>({...p,speed:+e.target.value}))}/></label><label>自动阅读间隔 <output>{preferences.delay} 秒</output><input type="range" min="1" max="8" value={preferences.delay} onChange={e=>setPreferences(p=>({...p,delay:+e.target.value}))}/></label><label>对白框不透明度 <output>{preferences.opacity}%</output><input type="range" min="55" max="100" value={preferences.opacity} onChange={e=>setPreferences(p=>({...p,opacity:+e.target.value}))}/></label><label>对白字号 <output>{preferences.fontSize}</output><input type="range" min="21" max="31" value={preferences.fontSize} onChange={e=>setPreferences(p=>({...p,fontSize:+e.target.value}))}/></label><label className="switch-label">界面动画与逐字显示 <input type="checkbox" checked={preferences.motion} onChange={e=>setPreferences(p=>({...p,motion:e.target.checked}))}/></label></div><p className="keyboard-help">空格 / → 推进　·　Esc 菜单　·　H 隐藏界面　·　F 全屏</p><button className="text-button" onClick={()=>setPreferences(defaults)}>恢复默认设置</button></>}
