...
export default function Home(){
 ...
 return <main onContextMenu={(event)=>{event.preventDefault();if(!setupOpen)setPanelOpen((open)=>!open);}}>
  ...
  <div className={`story-shell ${panelOpen?'panel-open':''}`}>
   ...
   <aside className="drawer" inert={!panelOpen}><button className="drawer-close" onClick={()=>setPanelOpen(false)} aria-label="关闭">×</button><p className="eyebrow">{activeBackground?.label||'人生阶段'} · 互动故事</p><h1>{state?.storyTitle||'正在生成故事'}<span>。</span></h1><p className="intro">{state?.world?.premise||selectedBackground?.premise||'选择人生阶段与同行者后，故事会根据模板生成。'}</p><div className="life-choice"><small>贯穿全文的选择</small><strong>{activeBackground?.lifeChoice.title||'尚未确定'}</strong><p>{activeBackground?.lifeChoice.question||''}</p></div><div className="separator"/><div className="cast-heading">同行者 <span>{String(cast.length).padStart(2,'0')}</span></div><div className="people">{cast.map((member)=><div className={`person ${state?.route===member.id?'chosen':''}`} key={member.id}><div className="portrait" style={{background:member.color}}>{member.symbol}</div><div><strong>{member.name}</strong><span>{member.identity}{state?.route===member.id?` · ${state.routeLabel||'已锁定'}`:''}</span><p>{member.detail}</p></div></div>)}</div><footer><span>{state?.relationshipType?state.routeLabel:'共同篇 · 正在形成倾向'}</span><span>{state?.selections.length||0} 次选择已记录</span></footer></aside>
