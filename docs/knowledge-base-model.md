...
当前一级组织单元不是一条回答，也不是完整情景，而是：

> 用户正在决定是否执行什么行动，或者在什么行动之间选择。

这个单元叫 `DecisionObject`（决策对象）。

知乎问题标题优先提供决策对象候选；回答正文是该对象下的案例与证据。系统用短规范说法统一“裸辞、辞职、离开公司”等不同说法，COEL 行为编码只提供上位语义锚点，再在同一对象内使用背景 embedding 排序案例。背景、选择和结果都不创建新的一级分类。

主链因此收敛为：

```text
知乎问题标题 → DecisionObject → 可选 COEL 行为锚点
知乎回答     → DecisionObject 下的案例 → 选择/结果/原文证据
用户输入     → DecisionObject → 同对象案例的情景排序
```

`DecisionEpisode` 仍可用于回答内部的“背景—行动—结果”分析，但它是二级案例结构，不再负责发现一级分类。后文关于情景聚类和分叉的内容保留为旧实验基线。
...
第一版采用“候选召回 → 字段对齐 → 自动质量门”的三步法：

1. 用“决策背景 + 决策点”做全文/向量召回，找出可能相近的已有情景；
2. 对齐少量核心字段：人生阶段、目标、关键约束、决策点、选项集合；
3. 按固定阈值检查相似度、决策点、来源数量和候选置信度，自动确认或排除。

可以先用一个简单的三档门槛，不让模型直接改写情景：

```text
高匹配：核心背景和决策点都一致 → 自动确认同一情景
中匹配：有相似处但存在缺口 → 自动排除本次归并，保留原因
低匹配：核心状态或决策点不一致 → 不归并，等待下一轮数据或算法版本
```
...
相似度高只能说明“值得比较”。只有通过相似度、决策点、候选置信度和来源快照门槛，关系库才自动写入 `CONFIRMED`；否则写入 `REJECTED` 并保留原因。关系库中的归属和原文证据仍然是权威。
...
当前本地工作库已导入 6,263 条 `bge-large-zh-v1.5:scenario-text-v2` 向量。自动批处理已生成 18 个情景簇，其中 15 个通过质量门并确认、3 个自动排除；行动分叉中 14 个确认、7 个排除。所有归属都保留 embedding 版本、算法版本和来源快照定位。
...
- 情景归属和正式分叉由确定性质量门自动确认或排除；模型只提供结构化候选，不能绕过校验直接写入事实。
...
GLM-4.7-Flash 当前官方标价为免费，适合承担绝大多数高频结构化任务；“免费”不等于没有配额、QPS 或排队限制，实际成本仍需统计失败重试、embedding、存储和自动质量门的拒绝率。以 10,000 条回答做全量处理前，先用 200–500 条黄金集校验结构正确率、证据覆盖率和升级比例，再锁定批量参数。

## 大量数据如何变成可检索的决策证据

数据库不负责生成“你应该选 A”的事实。它只把一万条回答压缩成可比较的证据，并在查询时找出与当前决策情景最相似的已自动确认记录；应用层可以再基于这些记录生成带证据的回答：
...
当前实现有原文层、`DecisionEpisodeCandidate`、候选情景召回层和自动归并层：

- 场景视角按回答标题精确分组；标题相同不代表前置状态和决策点相同。
- 单条记录只有启发式的“处境 / 决定 / 行动 / 结果”，还没有稳定的证据片段、选项、约束、目标、时间和叙述类型。
- embedding 只负责把候选缩小到可比较的小集合，不直接判定同一情景。
- `DecisionScenarioMembership` / `DecisionBranchMembership` 保存自动归类结果、阈值、算法版本和拒绝原因；通过质量门的状态为 `CONFIRMED`，不通过的状态为 `REJECTED`。
- 因此页面展示的是自动确认的正式情景；`PROPOSED` 只在显式诊断模式下产生，不是生产队列。
...
`semantic_status` 另行表示 `UNPROCESSED / CLASSIFIED / NEEDS_REVIEW / NO_UNIT`，不能与来源可用性或自动归并状态混用；`NEEDS_REVIEW` 在当前自动主流程中表示需要重新处理，不表示人工队列。
...
| `analysis_version` / `status` | 哪次抽取、待处理/确认/排除 |
...
这是知识库的核心，不是一个大 JSON，而是一组可检索、可追溯的结构化字段。
...
| `DecisionScenario` | 承载自动确认或自动排除的情景簇；状态和算法版本可追溯 |
| `DecisionBranch` | 承载自动确认或自动排除的行动分叉，并回链来源快照 |
| `DecisionScenarioMembership` | 候选到情景的可撤回归属，带相似度、算法、embedding 版本和自动门结果 |
| `DecisionBranchMembership` | 候选到分叉的证据归属，带自动门结果 |
...
- 候选、自动确认的正式知识、自动排除记录、来源原文四者在数据库和页面上分开；
...
ContentSnapshot
  ...
  → DecisionScenarioMembership（自动确认或自动排除）
  → DecisionBranchMembership（自动确认或自动排除）
```
