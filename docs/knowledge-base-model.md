...
数据库不直接从一万条回答生成一句“你应该选 A”。它先把数据压缩成可比较的证据，再在查询时生成一份有条件的比较报告：
...
1. 用“当前背景 + 决策点”召回候选规范情景；
2. 过滤掉主体、阶段、关键约束明显不一致的事件；
3. 在同一情景内按实际行动读取分叉；
4. 对每条分叉汇总短期/长期结果、代价、适用条件和相反案例；
5. 每个结论都附独立来源数、证据覆盖、结果缺失和冲突说明。

### 输出应该是什么

第一版输出“比较报告”，而不是无条件的推荐：
...
如果用户明确要求“给建议”，建议也必须写成条件句，例如“在现金流可承受、目标是快速积累项目经验的相似事件中，选择小公司的人常报告……”。不能把样本频次改写成成功概率，更不能把相关性写成因果结论。
...
数据不足时，系统应返回“没有足够可比证据”和缺失变量，而不是用相似但不相关的回答凑出答案。

这层只消费已经审核的 `DecisionEpisode / CanonicalScenario / BranchPoint`，不修改来源事实，也不把生成的建议写回事实表。生成报告可以缓存，但必须带检索条件、数据版本和证据 ID，便于复核。
