...
## 真实对话形态调查（2026-08-21，已完成）

- 用户提出“很多对话没法这么总结”，这与当前模型的风险一致：不是每个回答都应强制生成 `DecisionEpisode`。
- 本阶段调查目标从“设计更完整的字段”改为“先确认回答形态”。候选至少要分为：亲历决策、建议/经验法则、转述他人、假设/反事实、纯观点/信息、无法判断。
- 只有亲历或明确叙述的决策事件才进入决策经历候选；其他回答仍然必须保存为原文，可作为情景背景或检索证据，但不能伪装成个人路径。
- `ContentSnapshot` 是来源事实；`DecisionEpisodeCandidate` 是模型草稿；正式知识必须增加字段级证据和人工审核。三者不能用一个 `review_status` 混在一起。
- 来源问题标题不能直接作为正式情景。一个问题可以包含多个决策事件；多个问题也可能指向同一规范情景。标题聚合只能保留为探索投影。
- 更合理的 KISS 数据模型是“来源层 + 可选语义层 + 关系层”：回答可以只有来源记录；有明确决策时才创建经历；经历无法可靠归类时保留为未归类候选，不阻塞原文入库。
- 第一版应优先调查：叙述资格、决策点是否明确、行动是否具体、结果是否存在、证据是否可定位；不要先增加目标/约束/资源等大量强制字段。
- 数据库只读统计：`10397` 个快照、`2423` 个不同问题标题、`7221` 个启发式候选，所有候选仍是 `UNREVIEWED`。
- 当前候选字段并非都可靠：`1755/7221` 条候选没有结果文本；置信度只有 `71` 和 `93` 两档，说明现有启发式分数更像规则命中数，不足以代表语义可信度。
- 原文长度从 `77` 到 `132980` 字符，中位数约 `822`；回答形态和篇幅差异很大，不能用一个固定的“决策卡片”模板强制覆盖。
- 同一问题通常抓取约 `10` 条回答，因此“按标题聚合”只是来源批次聚合，不足以证明这些回答共享同一前置状态。
- 对全部正文做词项探测只能作为预筛，不是标注：`8144` 条含第一人称线索、`8954` 条含建议词、`7260` 条含结果/时间词、`2803` 条含营销线索；这说明“我”和“应该”大量共存，不能用单一关键词判断亲历或建议。
- `6214` 条同时出现第一人称和结果词，适合作为亲历复核候选；`1664` 条含建议词但没有第一人称线索，适合作为方法论/观点复核候选。二者都需要抽样人工核对，不能直接晋级。
- 对全部正文做的规则预分类（仅作抽样框，不是事实标签）得到：亲历决策候选 `4913`、亲历+建议混合 `781`、方法论/建议 `891`、其他/无法判断 `2305`、短观点 `555`、事实/比较分析 `95`、主题明确但形态不明 `350`、推广/聚合风险 `507`。
- 预分类的意义不是自动决定类型，而是证明模型需要允许多标签和“不抽取”：同一回答可能既有亲历又有建议，长文可能包含多个决策片段，广告/引用/转述不能直接成为人生路径。
- 人工复核样本显示：`现代家庭中...家务和育儿...` 是亲历叙事但启发式结果为空；`教培转行，该做什么？` 是亲历经历+路线建议的混合长文；`普通人最合适的养老方式...` 是短建议且没有经历；`富途控股的发展前景...` 是市场分析，不是人生决策经历；`想考研又不知道考什么专业好？` 含课程推广/他人引用，启发式却抽出了候选；`没工作和烂工作你该怎么选？` 是条件化观点，不能当作亲历路径。
- 因此“没有可总结的回答”不是异常，而是合法数据状态：保留原文，标注内容类型/证据资格，不创建决策经历。
- 按正文长度分层：短文 `<300` 字有 `2346` 条、其中 `1166` 条被启发式生成候选且仅 `411` 条带结果文本；中等长度 `6122` 条；长文 `>=2000` 字 `1929` 条、其中 `1781` 条生成候选。长文更容易混入多个事件、引用和推广，不能因为候选率高就视为质量高。
- 当前 `DecisionEpisodeCandidate` 还有明显字段复用：`1520` 条的 `decision == action`，`958` 条的 `context == decision`，`868` 条的 `action == outcome`。这证明四字段必须允许“未抽取”，并且需要字段级证据/审核，不能把有值当成有语义。
- 同一问题标题下常有 10 条回答，但候选的 `decision` 和 `action` 也常常是 10 个不同值（例如“事业单位值不值得去？”、“一人公司创业中，最大的风险是什么？”）。标题可以作为检索入口，不能作为情景主键。
...
回答快照
  → 原文片段
  → 决策事件（前置状态 → 决策点 → 方向 → 实际行动）
  → 结果观察（可有多个时间窗口）
  → 情景归属
  → 分叉关系
```

其中“亲历/建议/转述/假设”等只表示证据资格和叙述方式，不是用来代替事件关系的标签。建议和观点可以作为候选路径或背景证据，但不得伪装成已经发生的行动和结果。数据库首先要允许一条回答停留在来源层、片段层或未归类事件层，只有事件边界、原文证据和情景可比性都成立时，才向上形成正式知识。

用户进一步将单条回答的决策拆解收敛为三条主线：`决策背景 → 决策情景 → 决策结果`。这比把四个字段平铺成“处境/决定/行动/结果”更适合阅读和审核：背景描述选择前的状态，情景描述中段的选择与行动，结果允许有多个时间窗口。这里的“决策情景”是回答内的具体事件；跨回答合并后的对象继续称 `CanonicalScenario`，避免把事件和归纳分组混为一谈。

由此确定情景合并规则：只比较决策背景和决策点；不同实际行动形成同一情景下的分叉，不同结果保留为事件级观察。合并必须保存匹配字段、置信度、证据片段和审核状态，不能用标题、标签或结果相似度直接合并。

关于“如何让大量数据发挥作用”，决定采用决策情景检索层：先把回答压缩为规范情景、分叉和结果观察，再用用户当前的背景、决策点和关键约束召回相似证据，展示匹配字段、分叉、结果观察与原文链接。应用层可以基于这组证据生成回答，但不得脱离证据写建议、成功概率或因果结论，生成内容也不写回事实库。

用户进一步指出，规模化检索的核心应是“决策情景 embedding”。该判断成立，但 embedding 输入必须是规范化的“背景 + 决策点 + 关键约束/目标”，而不是完整回答、标签、最终行动或结果。向量用于召回候选，不能单独决定情景合并；合并仍需结构化字段阻断、原文证据和审核。

关于处理驱动，采用 Codex-Spark 作为语义提案驱动是合理的，但它不能成为数据库事实源。任务队列、批次边界、JSON/schema 校验、证据偏移核对、幂等、重试和审核必须由本地 Worker 控制；Spark 只负责有限任务的结构化理解。由于官方将 GPT-5.3-Codex-Spark 定位为 Codex 中的 research preview，驱动层必须支持替换，不能把后台无人值守流程绑定到 Spark 会话可用性。
