...
const effectKeys = ["cash", "health", "happiness", "knowledge", "connections", "career", "assets"] as const;

export function resourceContext(state: LifeState): ResourceContext {
  const cashBand = state.cash < 10 ? "生存线" : state.cash < 25 ? "紧张" : state.cash < 55 ? "稳定" : "宽裕";
  const healthBand = state.health < 15 ? "危险" : state.health < 35 ? "透支" : state.health < 65 ? "一般" : "良好";
  ...
  return {
    ...
    developmentConversion: Math.max(0.35, (state.cash < 10 ? 0.6 : state.cash < 25 ? 0.8 : 1) * (state.health < 15 ? 0.55 : state.health < 35 ? 0.78 : 1)),
  };
}
...
export function projectedLifeEndAge(state: LifeState, profile: Profile) {
  ...
  return Math.max(62, Math.min(100, Math.round(77 + healthContribution + happinessContribution + securityContribution + talentContribution)));
}
...
export function settleChoice(state: LifeState, profile: Profile, option: GameOption, roll = Math.random() * 100) {
  const context = resourceContext(state);
  const talentProtection = profile.talents.insight * 0.8 + profile.talents.luck * 0.7 + profile.talents.grit * 0.35;
  const effectiveRisk = Math.round(Math.max(3, Math.min(95, option.baseRisk + context.riskModifier - talentProtection)));
  const riskOccurred = roll < effectiveRisk;
  const raw = Object.fromEntries(effectKeys.map((key) => [key, Number(option.effects[key] || 0)])) as Required<Effect>;
