...
export async function readState(id:string):Promise<State|null> {if(!validId(id))return null;try{return JSON.parse(await readFile(path.join(root,`${id}.json`),'utf8'));}catch(e){if((e as NodeJS.ErrnoException).code==='ENOENT')return null;throw e;}}
