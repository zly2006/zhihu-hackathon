...
export type State = {version:1;nodes:StoryNode[];selections:Selection[];route:Route|null;pending:boolean;partial?:Partial<StoryNode>;memory:StoryNode['memory']};
export function initial():State {return {version:1,nodes:[],selections:[],route:null,pending:true,memory:{summary:'',facts:[]}};}
...
export function messages(state:State) {
 ...
 world:{...world,cast:cards},stage:state.nodes.length+1,total_stages:total,task:config.BEATS[state.nodes.length],
