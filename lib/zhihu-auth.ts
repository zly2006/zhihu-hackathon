...
type Session={id:string;expiresAt:number;oauthState:string|null;accessToken:string|null;tokenExpiresAt:number|null;profile:ZhihuProfile|null;error:AuthError|null;localMode?:boolean};
...
export function requireSession(request:NextRequest){const session=activeSession(request);return session&&(session.accessToken||session.localMode)?session:null;}
export function sessionStatus(request:NextRequest,session:Session){const config=configuration();const missingConfiguration=[!config.appId&&'ZHIHU_OAUTH_APP_ID',!config.appKey&&'ZHIHU_OAUTH_APP_KEY',!config.redirectUri&&'ZHIHU_OAUTH_REDIRECT_URI'].filter((value):value is string=>Boolean(value));const localMode=Boolean(session.localMode);return {configured:missingConfiguration.length===0,missingConfiguration,authorized:Boolean(session.accessToken||localMode),profile:session.profile,expiresAt:session.tokenExpiresAt?new Date(session.tokenExpiresAt).toISOString():null,error:session.error,localMode,localModeAvailable:localModeAvailable(request),developmentMode:localMode,playtest:localMode};}
export function status(request:NextRequest){const {session,created}=getOrCreate(request);return {session,created,payload:sessionStatus(request,session)};}
export function createLocalSession(request:NextRequest){
  if(!localModeAvailable(request))throw fail('LOCAL_MODE_UNAVAILABLE','本地模式仅在本机开发服务器上可用。');
  clearSession(request);
  const session=newSession();
  session.localMode=true;
  session.profile={...DEVELOPMENT_PROFILE};
  return session;
}
