...
import {DEVELOPMENT_MODE_MESSAGE,DEVELOPMENT_PROFILE,DEVELOPMENT_SESSION_ID,developmentModeEnabled} from './playtest';
...
function playtestSession():Session{return {id:DEVELOPMENT_SESSION_ID,expiresAt:Number.MAX_SAFE_INTEGER,oauthState:null,accessToken:'playtest-local',tokenExpiresAt:null,profile:DEVELOPMENT_PROFILE,error:null};}
const globalDevFlag=globalThis as typeof globalThis&{lamplightDevelopmentModeLogged?:boolean};
function developmentMode(){const enabled=developmentModeEnabled();if(enabled&&!globalDevFlag.lamplightDevelopmentModeLogged){globalDevFlag.lamplightDevelopmentModeLogged=true;console.warn('[auth]',DEVELOPMENT_MODE_MESSAGE);}return enabled;}
function fallbackSession(){return developmentMode()?playtestSession():null;}
function activeSession(request:NextRequest){const id=request.cookies.get(COOKIE_NAME)?.value;if(!id||!/^[A-Za-z0-9_-]{40,50}$/.test(id))return fallbackSession();const session=sessions.get(id)||null;if(!session)return fallbackSession();if(session.expiresAt<=Date.now()){sessions.delete(id);return fallbackSession();}if(session.tokenExpiresAt&&session.tokenExpiresAt<=Date.now()){session.accessToken=null;session.profile=null;session.tokenExpiresAt=null;session.error={code:'TOKEN_EXPIRED',message:'知乎登录已过期，请重新登录'};}return session;}
...
export function status(request:NextRequest){const {session,created}=getOrCreate(request);const config=configuration();const missingConfiguration=[!config.appId&&'ZHIHU_OAUTH_APP_ID',!config.appKey&&'ZHIHU_OAUTH_APP_KEY',!config.redirectUri&&'ZHIHU_OAUTH_REDIRECT_URI'].filter((value):value is string=>Boolean(value));const developmentModeActive=session.id===DEVELOPMENT_SESSION_ID;return {session,created,payload:{configured:missingConfiguration.length===0,missingConfiguration,authorized:Boolean(session.accessToken),profile:session.profile,expiresAt:session.tokenExpiresAt?new Date(session.tokenExpiresAt).toISOString():null,error:session.error,developmentMode:developmentModeActive,playtest:developmentModeActive}};}
