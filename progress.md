...
- **保留边界：** `zhurl` 仍是默认可替换传输；Cookie 只是本机请求凭据，不改变授权门禁和质量门。
...
- **合并规则：** 规范情景只由“可比较的决策背景 + 相同决策点”决定；行动差异进入分叉，结果差异留在结果观察，不参与情景合并。合并先召回、再字段对齐、最后通过自动质量门，并记录可撤回的 `ScenarioMembership`。
- **决策支持边界：** 大量数据先压缩为“规范情景 → 分叉 → 结果观察”的证据包，再按用户当前决策情景检索匹配；知识库返回相似情景、分叉、结果观察和原文证据，应用层可据此生成带证据的回答，但不生成成功概率或因果结论，也不写回事实库。
- **向量主轴：** 用户进一步明确应把“决策情景”作为 embedding 单位。已确定向量输入为“决策背景 + 决策点 + 已知选项/目标/约束”，不放入最终行动和结果；向量只做 Top-K 候选召回，结构化阻断、证据检查和自动质量门才决定情景归属。
- **处理驱动：** 语义处理采用 `processing_task → CodexSparkDriver → semantic_proposal → 确定性校验 → 自动确认/排除 → 事务写入`；Spark 不直接写库、不直接生成向量，所有提案保存模型/提示词版本、输入 hash 和证据片段，并保留可替换的 API 驱动接口。
...
- **下一步：** 增加 `candidate_embedding` 版本表；先用向量做 Top-K 召回，再做结构化字段阻断、证据校验和自动确认/排除，最后才写入情景归属。
...
- **数据库：** 新增 `decision_scenario_membership` 和 `decision_branch_membership`，保存候选归属、相似度、算法版本、embedding 版本、证据和自动状态；正常批处理通过门槛写 `CONFIRMED`，不通过写 `REJECTED` 并保存原因。
...
- **边界：** 这是“召回后的自动质量门”，不是模型单方面下结论；`PROPOSED` 只在显式 `--no-auto-confirm` 诊断模式出现。当前真实库已自动确认 15 个情景、14 个分叉，自动排除 3 个情景、7 个分叉。

### 阶段 31：全自动情景归并与分叉确认（已完成）

- **自动规则：** 新增 `auto-confirm-v1`，要求情景至少包含 2 个不同来源快照、决策点明确、最低相似度 `>=0.90`、所有候选置信度 `>=80`；通过写 `CONFIRMED`，否则写 `REJECTED` 并保存机器可读原因。
- **批处理：** `scripts/propose_scenario_clusters.py` 默认自动模式；`--dry-run` 只计算不写库，`--no-auto-confirm` 仅保留诊断提案，不是生产路径。增加 `--min-auto-similarity` 与 `--min-auto-confidence`，门槛可复现。
- **真实运行：** 6,263 条 `bge-large-zh-v1.5:scenario-text-v2` 向量生成 18 个情景簇、21 个分叉；15 个情景和 14 个分叉自动确认，3 个情景和 7 个分叉自动排除；确认归属 31 条，排除归属 7 条，原始知乎 URL/HTML 均可回链。
- **可追溯性：** 每条确认/排除归属保留 `confirmation_policy`、相似度、算法版本、embedding 版本、候选 ID 和快照证据；旧算法结果仍以 `SUPERSEDED` 保留，不覆盖原文。
- **验证：** 自动模式持久化测试、API 兼容测试、全量测试、Ruff、mypy 和数据库完整性检查通过。
