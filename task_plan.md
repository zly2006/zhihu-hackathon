...
数据库当前为 **10226 个唯一回答、10227 个快照、10502 个原始证据包**，另有 13664 个关键词候选、7094 个
决策经历候选和 72 次发现运行。全部原始信封均有知乎回答规范 URL 和非空 `raw_html`；质量门通过的 API
记录为 10496/10496。搜索/问题接口偶发 403 的关键词被跳过，没有绕过验证或写入演示数据。
