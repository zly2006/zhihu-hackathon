...
### Phase 26：扩充到 10000 条（等待授权恢复）

- [x] 将本机已有真实搜索响应全部安全扩充到 1585 条唯一回答
- [ ] 获得已完成网页验证或刷新后的知乎 Cookie，使搜索/问题/回答 API 恢复可用
- [ ] 按每批 50–100 条、限速、失败跳过、导入即去重的方式继续采集至 10000 条
- [ ] 对新增数据执行 URL、HTML、质量门、原始响应和候选派生检查
- **Status:** in_progress

**当前阻塞：** `search_v3` 与 `zhurl` 均返回知乎业务错误码 40352（网络环境异常，需要网页验证）。
继续请求不会增加数据，且可能延长风控窗口；不能绕过验证码或把演示数据填入数据库。
