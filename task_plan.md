...
### Phase 25：数据集扩容（当前 1500 条）

- [x] 在 40352 风控未解除期间，只从本机已有真实搜索响应筛选新增回答，不发起绕过验证的请求
- [x] 继续使用同一质量门、URL/HTML 原文契约和 ContentSnapshot 幂等导入
- [x] 将数据库从 1000 条扩充到 1500 条唯一回答快照
- [x] 为新增恢复记录保留 `capture_method=zhihu_search_result_promotion`，与完整三段式详情区分
- **Status:** complete

**扩容结果（2026-08-21）：** 新增 500 条质量通过的搜索结果恢复记录；数据库当前为
1500 个唯一回答、1500 个快照、1534 个原始证据包、1045 个决策候选。全部快照均有
知乎回答 URL 和非空 `raw_html`，其中 534 条为搜索结果恢复记录；知乎 API 恢复后应批量补齐其回答详情。
